...
if(wx<wy*2){
  wx=canvas.width =wx;
  wy=canvas.height=wx/2;
}
else{
  wx=canvas.width =wy*2;
  wy=canvas.height=wy;
}
...
var dx2 = -3;
var dy2 = -3;
var paddleHeight = canvas.height*0.1;
...
function buttonTest(){
ctx.beginPath();
ctx.rect(0, wy*0.9, wx*0.1, wy*0.1);
ctx.rect(wx*0.9, wy*0.9, wx*0.1, wy*0.1);
...
function drawPaddle() {
    ctx.beginPath();
    ctx.rect(paddleX, wy-paddleHeight, paddleWidth, paddleHeight);
    ctx.fillStyle = "#dd4700";
    ctx.fill();
    ctx.closePath();
}
function drawBricks() {
